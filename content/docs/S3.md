...
If you want to host the video segments on a self hosted S3 compatible [MinIO](https://min.io/) server, you get even more control over your data. A docker-compose deployment example is available in the [examples directory](https://github.com/owncast/owncast/blob/master/examples/docker-compose.yaml).
...
Make sure to allow access to the buckets file from your owncast domain. If your MinIO is deployed behind a traefik reverse proxy, make sure to add a middleware to your MinIO container:
...
For details, c.f. the [documentation](https://doc.traefik.io/traefik/middlewares/headers/#cors-headers), respective of the reverse proxy you use.
