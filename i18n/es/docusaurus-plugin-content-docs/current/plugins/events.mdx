...
module.exports = definePlugin({
  onTabContent(ctx) {
    if (ctx.slug === "stats") {
      return `<h1>Estadísticas en vivo para ${ctx.user?.displayName ?? "espectador"}</h1>`;
    }
    return "";
  },
  onPageContent(ctx) {
    return ctx.slug === "banner" ? "<p>¡Bienvenido!</p>" : "";
  },
});
```
...
Consulta [APIs de Owncast](/docs/plugins/apis#plugin-to-plugin-events) para la API de emisión.
