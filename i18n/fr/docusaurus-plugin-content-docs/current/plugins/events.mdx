...
module.exports = definePlugin({
  onTabContent(ctx) {
    if (ctx.slug === "stats") {
      return `<h1>Statistiques en direct pour ${ctx.user?.displayName ?? "visualiseur"}</h1>`;
    }
    return "";
  },
  onPageContent(ctx) {
    return ctx.slug === "banner" ? "<p>Bienvenue!</p>" : "";
  },
});
```
...
Voir [APIs d'Owncast](/docs/plugins/apis#plugin-to-plugin-events) pour l'API d'émission.
