...
module.exports = definePlugin({
  onTabContent(ctx) {
    if (ctx.slug === "stats") {
      return `<h1>Live-Statistiken für ${ctx.user?.displayName ?? "Zuschauer"}</h1>`;
    }
    return "";
  },
  onPageContent(ctx) {
    return ctx.slug === "banner" ? "<p>Willkommen!</p>" : "";
  },
});
```
...
Siehe [Owncast APIs](/docs/plugins/apis#plugin-to-plugin-events) für die Emit-API.
