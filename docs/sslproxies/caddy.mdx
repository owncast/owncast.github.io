...
<Tabs>
  <TabItem value="apt" label="Using APT (recommended)" default>
...
[Read the Caddy install steps for using apt](https://caddyserver.com/docs/install#debian-ubuntu-raspbian) for more details.
  ...
  <TabItem value="manually" label="Download manually">
  ...
  1. Unarchive the file: `tar -xvzf caddy_<version>_linux_amd64.tar.gz`
  1. You're likely to want to setup Caddy as a system service to automatically start in the background.  [Learn how to do this](https://caddyserver.com/docs/install#linux-service).
...
<Tabs>
  <TabItem value="single-command-line" label="Single command line" default>
...
Replace `owncast.example.com` with the public hostname of your Owncast server like `watch.owncast.online` for example.

  </TabItem>

</Tabs>

If you specify `owncast.example.com` without a protocol or a port, it will attempt to use the default `http` and `https` ports (80 and 443). Since these are [_privileged ports_](https://www.w3.org/Daemon/User/Installation/PrivilegedPorts.html#:~:text=Priviliged%20ports,has%20put%20up%20for%20you.), you will need to run caddy with `sudo` or as `root`.
...
You should now be able to access your Owncast server by visiting https://owncast.example.com instead of http://owncast.example.com:8080.
