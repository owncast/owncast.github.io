...
Before writing the archive, both SDKs run your built plugin through `owncast-plugin-test --load-only`: the same install-time load path a real server runs, covering `register()`, manifest-runtime agreement, and permission-gated subscriptions. Packaging fails if a real Owncast server would refuse to load the plugin, for example a fediverse handler without `fediverse.inbound` declared. The check needs the host test binary, which the JS SDK fetches during `npm install` and the Python SDK downloads on first use.
