...
This page takes you from nothing to a plugin running on your Owncast server. Plugins can be written in **JavaScript/TypeScript** or **Python** — pick the tab for your language below. If you're not sure which to use, see [Choosing an SDK](/docs/plugins/sdks); the two have full feature parity and differ mainly in plugin size.
...
- Python 3.9 or newer, and `pip` or [`uv`](https://docs.astral.sh/uv/) to install the `owncast-plugin-sdk` toolchain.
...
Install the SDK to get the `owncast-plugin-py` CLI (it downloads and caches the build toolchain on first use):

```sh
pip install owncast-plugin-sdk    # or:  uv tool install owncast-plugin-sdk
```

There's no scaffolder for Python yet, so create the directory by hand:
...
│   └── plugin.py            your code
└── __tests__/
    └── plugin.test.json     optional scenario test
```

A minimal `plugin.manifest.json`:
...
See the [handlers reference](/docs/plugins/handlers) for everything you can hook into, and the [APIs reference](/docs/plugins/apis) for every `owncast.*` method.
...
- [Choosing an SDK](/docs/plugins/sdks) and its [JavaScript](/docs/plugins/sdks/javascript) / [Python](/docs/plugins/sdks/python) pages for the full language-specific reference.
