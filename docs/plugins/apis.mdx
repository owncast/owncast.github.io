...
Emit a custom event that other plugins can subscribe to. Subscribing plugins receive it through their custom-event handler: see the [handlers reference](/docs/plugins/events#custom-events). Namespace your event types with your plugin name to avoid collisions.
...
The current live stream state: `{ online, startedAt?, title?, viewers, ... }`.
...
Static server info: `{ name, version, summary, ... }`.
...
Read the output and transcoding configuration: `{ latencyLevel, codec, variants }`.
