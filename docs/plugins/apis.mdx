...
| Method                       | Returns                                              |
| ---------------------------- | ---------------------------------------------------- |
| `sql.exec(sql, params?)`     | `{ ok, error?, rowsAffected, lastInsertId }`         |
...
In Python `queryRow` is `query_row`, rows come back as dicts, and `query_row` returns `None` when nothing matched. An error throws in JavaScript and raises `RuntimeError` in Python, so you don't have to check `ok` yourself. Parameters are `null`/`None`, booleans, numbers, or strings. Anything else is refused.
