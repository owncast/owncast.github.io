...
The Owncast plugin runtime exposes a single global, `owncast`, with the host functions your plugin can call. Most methods require the matching permission in your manifest, and calling without it throws a clear error. A few are **ambient** and need no permission: timers, reading bundled assets, and `owncast.config.get`.
...
Clear the current viewer's session cookie on this response — a sign-out. Your plugin still controls the redirect (and may bounce on to the provider's own logout).
...
Upload a file to Owncast's public file area. Returns `{ url }` (for example, `https://your-server.example/public/plugins/my-plugin/badge.png`), usable in image tags, action buttons, fediverse posts, and so on.
...
Read one of your plugin's [manifest-declared `config`](/docs/plugins/manifest#config-admin-configurable-settings) settings. Returns the admin-set value when present, otherwise the declared default, already parsed to its declared type. For an unknown key (or one with no value) it returns `fallback`.
...
Whether fediverse federation is enabled and under what handle: `{ enabled, username, isPrivate }`.
