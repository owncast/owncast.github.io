...
For [`auth.gate`](/docs/plugins/auth) plugins, drives the [`onAuthCheck`](/docs/plugins/events#auth-check) handler directly with a resolved viewer identity and asserts the verdict:
...
`tabContent` and `pageContent` take a `slug` and an optional `user`; all four accept `expect.body` (exact) or `expect.bodyContains`.
