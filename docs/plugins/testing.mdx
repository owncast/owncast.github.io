...
:::new[Plugin testing requires Owncast v0.3.0]
...
Use `expect.bodyBase64` for an exact byte comparison of a binary response. It
accepts standard base64 with or without padding. `expect.body` and
`expect.bodyContains` compare response text.
