...
Run them with `owncast-plugin-py test my-plugin`. Python uses the JSON format — there's no script-based runner.

</TabItem>
</Tabs>

Running the tests builds your plugin, then runs every scenario file under `__tests__/`. The rest of this page is the scenario reference — the same data model whichever SDK you use.
