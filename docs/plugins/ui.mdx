---
title: Contributing web UI with Plugins
...
<img
  src="/docs/img/plugin-admin-page.png"
  alt="A plugin's admin page rendered as a tab inside the admin, alongside its Instructions and Permissions tabs"
  width="80%"
/>
...
The admin UI deduplicates tabs by the resolved iframe URL, not by title. `/admin` and `/admin/*` both resolve to `/admin/`, so this pair produces one visible tab that gates the whole subtree. A pair like `/admin` and `/admin/api/*` resolves to two different URLs and produces two tabs.
...
<img
  src="/docs/img/plugin-action-buttons.png"
  alt="A row of plugin-contributed action buttons beneath the stream on the viewer page, next to the built-in Follow and Notify buttons"
  width="85%"
/>
...
<img
  src="/docs/img/plugin-viewer-tabs.png"
  alt="Plugin-contributed tabs on the viewer page, shown alongside the built-in About and Followers tabs"
  width="75%"
/>
See [Styling plugin UI](#styling-plugin-ui) for the baseline and the helper classes. Plugin tabs are
appended after the built-ins in declaration order. The React key is derived from the plugin's slug
so a tab unmounts only when its source plugin is disabled or removed.
