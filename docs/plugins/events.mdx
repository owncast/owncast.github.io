...
This is a passive event. Calling `chat.send` from this callback can feed the
callback again, so use `chat.message.received` for handlers that react to user
input and post replies. No permission is required to subscribe.
