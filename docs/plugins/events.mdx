...
Only fires for the enabled [`auth.gate`](/docs/plugins/auth) plugin, and only on a viewer's `/` page load — never on the hot path (video segments, the API, chat). It's the **re-validation hook**: the host has already verified the viewer's session cookie and resolved their identity; your handler decides whether that session should continue. Optional — omit it and a valid cookie is enough until it expires.
...
Requires `auth.gate`. **Fails closed**: if the handler errors or times out, the host treats that page load as a deny. Because the check runs only on `/`, a viewer whose access you revoke keeps any open tab working until they reload or the cookie expires — the session `ttl` is the hard backstop.
...
interface SSEConnectionEvent {
  ...
  user?: ChatUser; // present only when the connection carried a chat identity
}
```
