...
`user` carries the **full sender identity**, so key per-user state on the stable `user.id` and gate moderator-only behavior on `user.scopes` (e.g. `"MODERATOR"`) rather than matching the display name. To reply privately to the sender, use the chat reply-to API (see [Owncast APIs](/docs/plugins/apis#chat-replyto)).
...
### Viewer-visible chat: `chat.message.broadcast`

Fires for every chat event viewers receive, including human messages, plugin
responses, system messages, and chat actions. The `body` is sanitized,
rendered HTML. Use this event for passive consumers such as overlays.
...
This is a passive event. Do not declare `chat.send` in a plugin that handles
`chat.message.broadcast`; use `chat.message.received` for handlers that react
to user input and post replies. No permission is required to subscribe.
...
See [Owncast APIs](/docs/plugins/apis#plugin-to-plugin-events) for the emit API.
...
| `chat.message.broadcast` | `ChatMessageBroadcast`             | none                                               |
