---
title: Serving HTTP via Plugins
...
Code is shown for both SDKs. See [JavaScript](/docs/plugins/sdks/javascript) or [Python](/docs/plugins/sdks/python) for install and setup.
...
```mermaid
flowchart LR
    Req([Request to<br/>/plugins/my-plugin/X]) --> Admin{Path matches a<br/>manifest.admin.pages key?}
...
The `manifest.admin.pages` key match at the top is covered in [UI: Admin pages](/docs/plugins/ui#admin-pages). From the perspective of HTTP serving, it is a 401-before-your-handler-runs filter applied to paths matching one of the object's keys.
...
- Response headers are filtered through an allowlist. You can set `Content-Type`, `Content-Encoding`, `Content-Language`, `Cache-Control`, `Set-Cookie`, `Location`, `ETag`, `Last-Modified`, `Vary`, `Link`, and CORS (`Access-Control-*`) headers. Owncast-owned headers (`Server`, `Content-Security-Policy`, `Strict-Transport-Security`, `X-Frame-Options`) are blocked.
...
Endpoints are public by default. To make something admin-only, either check whether the request is authenticated inside your handler and return `401` when it isn't, or add its path glob as a key in `manifest.admin.pages` and let the host gate it for you (see [UI: Admin pages](/docs/plugins/ui#admin-pages)).
...
Push from any handler, for example, from a viewer-visible chat handler, by calling `owncast.sse.send(channel, event, data)`:
...
`onChatMessageBroadcast` / `@plugin.on_chat_message_broadcast` includes
human messages, plugin responses, system messages, and actions. Its `body`
is sanitized rendered HTML. Use `textContent` for untrusted text; only insert
this host-sanitized `body` as HTML when reproducing Owncast's chat rendering.
...
Sends are fire-and-forget. The call returns immediately and never blocks, even if no one is connected or a client is slow. Slow clients drop frames rather than stall your plugin. There are also SSE-connection lifecycle events (a viewer's stream opening and closing) you can subscribe to: see the [handlers reference](/docs/plugins/events#sse-events).
...
- If the channel matches a key in `admin.pages`, it's auth-gated like any admin route. Handy for an admin-only stats stream.
...
The plugin subscribes to viewer-visible chat and pushes each message to the
`overlay` SSE channel:
