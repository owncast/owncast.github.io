...
| --- | --- |
| `manifest` | The packaged `plugin.manifest.json`, unchanged. |
...
A language SDK derives subscriptions and commands from registered handlers. A native module has no SDK to do that work, so declare `subscriptions` and `commands` in `plugin.manifest.json`. Returning the injected manifest from `register` reports those declarations to Owncast.
...
The load check calls `register`, compares the returned identity and permissions with the packaged manifest, validates permission-carrying subscriptions, and instantiates the real host imports. Fix any load failure before creating the `.ocpkg`.
