...
When an `auth.gate` plugin is enabled, the **entire web server is gated**, not just the HTML page. The viewer page (`/`), the video (`/hls/*`), chat (`/ws`), and the API all sit behind the gate. Owncast's own admin pages are the exception: they stay behind the admin login rather than the viewer gate, so an admin can always reach the controls.
...
When distributing your video stream directly from your server, the gate is airtight: every byte flows through Owncast. With Object Storage or CDN, playlists are rewritten to absolute remote URLs and segments are fetched directly from the bucket, so the gate never sees those requests. Gating still stops an anonymous visitor from _discovering_ the segment list, but a _leaked or shared_ segment URL stays fetchable. **Gate + local distribution is airtight. Gate + Object Storage is good friction, not airtight.**
:::
...
| ------------------------------------------------ | ---------------------------------------------------------------------- | -------------------------------------------------------------------- |
| **Every request** (`/hls/*`, images, `/ws`, API) | Verify cookie signature + expiry                                       | valid → pass, missing/invalid → redirect to login                    |
...
flowchart TD
    ...
    Cookie -->|"missing or invalid"| Login["Core redirects to your login screen<br/>/plugins/&lt;slug&gt;/?return_to=…"]
    ...
    Register --> Grant["owncast.auth.grantSession<br/>core mints and attaches the signed cookie"]
...
- External-API-token routes, where a valid Bearer client carries no cookie and gating would bounce it into a login page.
...
- **The signing secret is core's responsibility.** It's auto-generated on first use and persisted in config. Rotating it invalidates every session (a panic button). Plugin authors never touch it, and it's separate from any OAuth _client_ secret, which is your plugin's config concern.
