...
- **The Owncast host is the gatekeeper and session authority.** It owns the session cookie, enforces the gate on every request, and never lets your plugin near the per-request hot path.
...
The cookie is a signed envelope carrying the user's existing Owncast access token plus a session expiry. The Owncast host owns it end to end: it reserves the cookie name, signs it with a host-held secret, and attaches it to the response. Your plugin never sees or sets the token, so it can't forge or leak one. (This is also how chat picks up the viewer's identity automatically. See [Chat identity](#chat-identity) below.)
