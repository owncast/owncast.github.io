...
Passthrough mode tells Owncast to send your broadcaster's video straight through without re-encoding it. Because the server skips encoding entirely, hardware usage drops to almost nothing. This is the single biggest reduction you can make if your server simply cannot keep up. See [video passthrough](/docs/video/#video-passthrough) for how to enable it.

**Drawback**: With passthrough you can no longer offer multiple quality options, so viewers on slower connections have no lower-quality stream to fall back to. Passthrough can also increase latency, and in some environments it produces video that not all players can handle.
