...
<img src="/images/moderator-menu.png" alt="The moderator menu that appears when hovering over a chat message, with options to remove the message, ban the user, and view more info about them" width="50%" />
...
![The Messages tab of the admin, with the eyeball visibility toggle on the right of a chat message](/docs/img/chat-moderation-hide-message.png)

### Bulk changing of visibility

![The Messages tab with several messages selected by checkbox and the show and hide buttons above the list](/docs/img/chat-moderation-bulk-hide-messages.png)
...
![The Users tab of the admin, showing a user row with the option to ban the user](/docs/img/user-moderation-ban-user.png)

Alternatively, you can click on the user display name and bring up the user info modal and ban them from there.

![The user info modal opened from a display name, with a button to ban the user](/docs/img/user-moderation-ban-user-modal.png)

### Banning vs deleting

Banning is reversible, since you can un-ban the user later. The same menu also offers _Delete user_, which is permanent. It removes the user and all of their data, including their messages, and cannot be undone.
...
- **Require Authentication**: only [authenticated users](/docs/chat/chat-authentication) and moderators may send messages.

## Rate limiter

Chat has a built-in rate limiter to prevent spam. It allows roughly one message every two thirds of a second. A user who floods faster than that is blocked from sending messages for 10 seconds and is told so. Moderators are never rate limited. You can turn the limiter off with the _Spam Protection_ toggle in the chat settings.
