...
**What peak network speed does your server need?**

Your server must be able to sustain the combined outbound bitrate to all viewers at the same time. In this scenario: (20 viewers × 5000kbps) + (5 viewers × 1500kbps) = **107.5 Mbps**. Your hosting plan needs to support at least this much outbound throughput.
...
All 25 viewers are receiving the same 5000kbps stream: 25 × 5000kbps = **125 Mbps** of sustained outbound throughput.
...
Since viewers are served from S3, your server doesn't send video to viewers at all. It only needs enough outbound bandwidth to upload one copy of each quality stream to S3: 5000kbps + 1500kbps = 6500kbps = **6.5 Mbps**. This stays the same whether you have 0 viewers or 1,000. Your S3 provider handles the viewer-facing throughput, which in this scenario peaks at **107.5 Mbps**, but that is their infrastructure, not yours.
...
### What peak network speed does my server need?

**Formula**: `bitrate in kbps × viewers ÷ 1,000 = Mbps`
...
This is the sustained outbound throughput your server needs during a stream. If you offer multiple quality options, calculate each quality separately and add them together. Most VPS providers offer 1 Gbps ports, which supports roughly 200 viewers at 5000kbps. If your viewers exceed your server's network capacity, they will experience buffering. Using [S3-compatible storage](/docs/storage) eliminates this limit from your server.

### How much CPU will Owncast use?

These are rough examples from measured tests, not guarantees. Actual CPU use varies by codec, bitrate, resolution and encoder preset.
...
As a rough rule of thumb from measured examples, a single CPU core can often handle one transcoded output quality at 30fps, but this varies with codec, bitrate, resolution and encoder preset. Passthrough mode (no transcoding) uses very little CPU regardless of bitrate. All CPUs are different in performance across hardware and hosting providers, so no guarantee can ever be made. You will need to experiment to find out what works best for your needs.
...
Higher frame rates require more CPU. Reducing frame rate is an effective way to lower CPU usage. These multipliers are rough measured examples and will vary by codec, bitrate, resolution and encoder preset:
...
Live video needs very little disk space. Owncast writes HLS video segments to disk temporarily and cleans them up as the stream progresses, so you don't need to plan for video storage.

The `data/` directory does persist and grow over time. It holds the Owncast database and its hourly backups, custom emoji, public files, viewer metrics, GeoIP data and, starting with Owncast 0.3.0, installed plugins and their data. Give it room to grow and include it in your [backups](/docs/backups).
...
**Recommendation**: Offering a second, lower quality option (1000-2000kbps) alongside your high quality one (4000-6000kbps) helps viewers on slow or mobile connections. That said, Owncast ships with a single variant by default, and the [video documentation](/docs/video) recommends starting with one variant and testing before adding more. Add a low quality option once your setup works and your CPU has headroom.
