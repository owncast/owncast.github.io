...
1. Visit your Owncast admin and go to Configuration > Server Setup. On the Server Config tab, under Advanced Settings, set the "Websocket host override" to the origin URL you created in step 1. For example, `https://owncast-origin.example.com`. The field takes a regular `http://` or `https://` URL and the web player converts it to a websocket connection. If you don't perform this step your chat will no longer be accessible for CDN providers who do not support passing through websockets. See the [server setup documentation](/docs/configuration/server-setup) for more about these settings.
...
1. In the Owncast admin go to Configuration > Server Setup. On the Server Config tab, under Advanced Settings, set the "Serving Endpoint" to the full URL of your CDN deployment. For example, `https://cdn.example.com`. See the [server setup documentation](/docs/configuration/server-setup) for more about these settings.
