...
async function buildManifest() {
  ...
  for (const [file, m] of Object.entries(meta.pages || {})) {
    const labels = listOf(await api('/api/pages/labels', { body: { pageId: m.pageId } })).map(
      l => l.name,
    );
...
// Turn the exported Markdown + manifest into a Docusaurus "/dev-docs" docs instance:
...
function emitDocusaurus(manifest, spaceSlug) {
  ...
  for (const p of manifest) {
    ...
    // /dev-docs, so rewrite those links on emit.
    body = body.replace(/\]\(\/dev\//g, '](/dev-docs/');
    ...
    withSlug.push({ ...p, slug: `/dev-docs/${slug}` });
  }
...
(async () => {
  ...
  if (process.env.OUT_DIR === undefined)
    console.warn(`Raw responses, if any, are in ${OUT}/debug/.`);
  ensureScaffold();
